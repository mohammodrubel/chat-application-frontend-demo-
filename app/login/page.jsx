"use client";
import React from "react";
import { useForm } from "react-hook-form";
import { useDispatch } from "react-redux";
import { setUsers } from "../src/Redux/featchers/auth/authSlice";
import verifyToken from "../src/Redux/lib/verifyToken";
import { useLoginMutation } from "../src/Redux/featchers/auth/authApi";

function Register() {
  const [loginUser, { data: userInfo, isLoading }] = useLoginMutation();
  const { register, handleSubmit } = useForm();
  const dispatch = useDispatch();

  const onSubmit = async (data) => {
    try {
      const res = await loginUser(data).unwrap();
      const user = verifyToken(res?.data?.data?.accessToken);
      dispatch(setUsers({ user: user, token: res?.data?.data.accessToken }));
     alert('login successfull')
    } catch (error) {
      console.log(error);
    }
  };

  return (
    <div className="h-screen flex justify-center items-center bg-white">
      <div className="w-full sm:w-[80%] md:w-[60%] lg:w-[50%] max-w-md p-8 rounded-lg shadow-lg">
        <h3 className="text-center text-bold text-2xl underline mb-4">Login</h3>
        <form onSubmit={handleSubmit(onSubmit)} className="text-center">
          <input
            type="email"
            placeholder="User Email"
            className="w-full p-2 mb-2 border rounded-lg"
            {...register("email", { required: true, maxLength: 20 })}
          />{" "}
          <br />
          <input
            type="password"
            placeholder="User Password"
            className="w-full p-2 mb-2 border rounded-lg"
            {...register("password", { required: true, maxLength: 20 })}
          />{" "}
          <br />
          <br />
          <input
            type="submit"
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          />
        </form>
      </div>
    </div>
  );
}

export default Register;
